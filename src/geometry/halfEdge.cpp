...
{
  for (std::list<HalfEdge*>::iterator it = _halfEdges.begin(); it != _halfEdges.end(); ++it) {
    if (*it == edge) {
      _halfEdges.erase(it);
      *modified = true;
      break;
...
{
  auto& nextEdgeAdress = _halfEdges.end();
  ...
  HalfEdge* halfEdge = *nextEdgeAdress;// std::prev(_halfEdges.end(), 6);
