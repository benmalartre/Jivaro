...
void ImGui::PushColumnsBackground()
{
    ...
    if (columns->Count == 1)
        return;
    columns->AmnSplitter.SetCurrentChannel(window->DrawList, 0);
...
void ImGui::PopColumnsBackground()
{
    ...
    if (columns->Count == 1)
        return;
    columns->AmnSplitter.SetCurrentChannel(window->DrawList, columns->Current + 1);
    PopClipRect();
}
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiColumnsFlags flags)
{
    ...
    if (columns->Count > 1)
    {
        columns->AmnSplitter.Split(window->DrawList, 1 + columns->Count);
        columns->AmnSplitter.SetCurrentChannel(window->DrawList, 1);
...
void ImGui::EndColumns()
{
    ...
    {
        PopClipRect();
        columns->AmnSplitter.Merge(window->DrawList);
    }
