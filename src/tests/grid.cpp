...
// thread task
void TestGrid::_FindHits(size_t begin, size_t end, const GfVec3f* positions, 
...
bool TestGrid::_CompareHits(const VtArray<bool>& hits, const  VtArray<GfVec3f> points)
{
...
void TestGrid::_TraverseStageFindingMeshes(UsdStageRefPtr& stage)
{
...
void TestGrid::_AddAnimationSamples(UsdStageRefPtr& stage, SdfPath& path)
{
...
void TestGrid::InitExec(UsdStageRefPtr& stage)
{
  if (!stage) return;
...
void TestGrid::UpdateExec(UsdStageRefPtr& stage, float time)
{
...
void TestGrid::TerminateExec(UsdStageRefPtr& stage)
{
  if (!stage) return;
