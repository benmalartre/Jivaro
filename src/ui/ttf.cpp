...
//
//    scope
//        - makes a new scope explicit
//        - used like scope { ... }
//        - really does nothing, just there for documentation/consistency purposes
//
//    debug_scope
//        - allows defining of debug code paths
//        - use like debug_scope { debug_code_here(); ect(); } or debug_scope debug_code_here();
// ---------------------------------------------------------------------------------------------------------------------------
...
#    define debug_scope
#else
#    define debug_scope while(false)
#endif
...
Bound::Bound(vec2f p_, vec2f d_)
    : p(p_), d(d_)
{}

TriEdge::TriEdge(size_t v0_, size_t v1_)
    : v0(v0_), v1(v1_), inUse(true)
{}
...
std::vector<MeshVertex>::iterator Mesh::begin()
{
    return verts.begin();
}
std::vector<MeshVertex>::const_iterator Mesh::begin() const
{
    return verts.begin();
}
std::vector<MeshVertex>::iterator Mesh::end()
{
    return verts.end();
}
std::vector<MeshVertex>::const_iterator Mesh::end() const
{
    return verts.end();
}
...
bool Triangulator::Intersect2(vec2f p, vec2f r, vec2f q, vec2f s) const
{
    ...
    if (rxs > 0.0f)
    {
        return tt > 0.0f && tt < rxs && ut > 0.0f && ut < rxs;
    }
    else if (rxs < 0.0f)
    {
        return tt < 0.0f && tt > rxs && ut < 0.0f && ut > rxs;
    ...
    {
        return tt == 0.0f && ut == 0.0f && Intersect(p.x, p.x + r.x, q.x, q.x + s.x) &&  Intersect(p.y, p.y + r.y, q.y, q.y + s.y);
    }
}
...
bool LineSegmentCompare(const LineSegment& a, const LineSegment& b)
{
...
void Triangulator::TriangulateEdges()
{
    ...
    // sort
    std::sort(seg.begin(), seg.end(), LineSegmentCompare);
...
void Triangulator::TraceContour(CItr begin, CItr end)
{
    ...
    {
        TraceContourHelper(i, pp, ip, ppc);
    }
...
void Triangulator::TraceContourInter(CItr begin, CItr end)
{
    ...
    if ((end - begin) < 2)
    {
        throw InvalidFontException("Cannot trace contour, contour length too small.");
    }
...
CodePoint::CodePoint(ulong codeId_)
    : codeId(codeId_), platformId(3), encodingId(1), languageId(0)
{}

CodePoint::CodePoint(ulong codeId_, ushort platformId_, ushort encodingId_, ushort languageId_)
    : codeId(codeId_), platformId(platformId_), encodingId(encodingId_), languageId(languageId_)
{}
...
Font::Font(std::string flnm)
{
    ...
    if (file.fail())
    {
        throw FileFailure(flnm);
    }

    if (length == 0)
    {
        throw FileLengthError(flnm);
    }
...
ushort Font::GetGlyphIndex(CodePoint codePoint) const
{
    ...
    if (version != 0)
    {
        throw VersionException("Table 'cmap' version is unsupported.");
    }
...
Font::FRange Font::MapGlyphIndexToRange(ushort glyphIndex) const
{
    ...
    // sanity check
    if (glyphIndex > numGlyps)
    {
        throw InvalidFontException("Invalid glyph mapping, attempting to map a glyph index that exceeds the maximum number of glyphs in the font.");
    }
...
ushort Font::GetGlyphIndexF0(FItr itr, ushort langId, ulong codeId) const
{
    ...
    if (format != 0)
    {
        throw FontException("Internal error, calling GetGlyphIndexF0() on a 'cmap' table that isn't format 0.");
    }
...
ushort Font::GetGlyphIndexF4(FItr itr, ushort langId, ulong codeId) const
{
    ...
    if (format != 4)
    {
        throw FontException("Internal error, calling GetGlyphIndexF4() on a 'cmap' table that isn't format 4.");
    }
    ...
    if (ReadUSHORT(endCodes + (segCount - 1) * 2) != 0xffff)
    {
        throw InvalidFontException("Last end code of format 4 'cmap' table is not 0xffff.");
    }

    if (ReadUSHORT(endCodes + (segCount) * 2) != 0)
    {
        throw InvalidFontException("Variable 'reservePad' following end code data of format 4 'cmap' table is not 0.");
    }
    ...
    while (imin < imax)
    {
        ...
        if (codeId > ReadUSHORT(endCodes + imid * 2))
        {
            imin = imid + 1;    // just need to check for greater than, so only need to read end code
        }
        else
        {
            imax = imid;
...
ushort Font::GetGlyphIndexF6(FItr itr, ushort langId, ulong codeId) const
{
    ...
    if (format != 6)
    {
        throw FontException("Internal error, calling GetGlyphIndexF6() on a 'cmap' table that isn't format 6.");
    }
...
ushort Font::GetGlyphIndexF10(FItr itr, ushort langId, ulong codeId) const
{
    ...
    if (format != 10.0f)
    {
        throw FontException("Internal error, calling GetGlyphIndexF10() on a 'cmap' table that isn't format 10.0.");
    }
...
ushort Font::GetGlyphIndexF12(FItr itr, ushort langId, ulong codeId) const
{
    ...
    if (format != 12)
    {
        throw FontException("Internal error, calling GetGlyphIndexF12() on a 'cmap' table that isn't format 12.");
    }
    ...
    while (imin < imax)
    {
        ...
        if (codeId > ReadULONG(itr + imid * 12 + 8))
        {
            imin = imid + 1;    // just need to check for greater than, so only need to read end code
        }
        else
...
ushort Font::GetGlyphIndexF13(FItr itr, ushort langId, ulong codeId) const
{
    ...
    if (format != 13)
    {
        throw FontException("Internal error, calling GetGlyphIndexF13() on a 'cmap' table that isn't format 13.");
    }
...
GlyphMetrics Font::GetGlyphMetrics(ushort glyphIndex) const
{
    ...
    {
        throw VersionException("Invalid 'hhea' table version.");
    }

    if (metricDataFormat != 0)
    {
        throw VersionException("Invalid 'hhea' metricDataFormat.");
    }
...
VGlyphMetrics Font::GetVGlyphMetrics(ushort glyphIndex) const
{
    ...
    if (version != 0x00010000)
    {
        throw VersionException("Invalid 'vhea' table version.");
    }

    if (metricDataFormat != 0)
    {
        throw VersionException("Invalid 'vhea' metricDataFormat.");
    }
...
vec2f Font::GetKerning(CodePoint cp0, CodePoint cp1, bool hk) const
{
    ...
    if (version == 0)
    {
        ...
        {
            itr += ParseMSKernTable(itr, g0, g1, hk, kv);
        }
    }
    else if (version == 1)
    {
        ...
        for (ulong i = 0; i < nTables; ++i)
        {
            itr += ParseAppleKernTable(itr, g0, g1, hk, kv);
        }
    }
...
float Font::ParseKernTableF0(FItr itr, ushort g0, ushort g1) const
{
    ...
    if (ReadULONG(itr + imin * 6) == key)
    {
        return static_cast<float>(ReadSHORT(itr + imin * 6 + 4));
    }
    else
    {
        return 0.0f;
...
float Font::ParseKernTableF2(FItr itr, ushort g0, ushort g1) const
{
    ...
    if (g0 >= leftFirstGlyph && g0 < leftFirstGlyph + leftNGlyphs)
    {
        leftClassOffset = ReadUSHORT(itr + (g0 - leftFirstGlyph) * 2);
    }
    ...
    if (g0 >= rightFirstGlyph && g0 < rightFirstGlyph + rightNGlyphs)
    {
        rightClassOffset = ReadUSHORT(itr + (g0 - rightFirstGlyph) * 2);
    }
...
ushort Font::ParseMSKernTable(FItr itr, ushort g0, ushort g1, bool hk, vec2f& kv) const
{
    ...
    {
        case 0:
            km = ParseKernTableF0(itr, g0, g1);
            break;

        case 2:
            km = ParseKernTableF2(itr, g0, g1);
            break;
    }
    ...
    if ((horizontal && !xstream) || (!horizontal && xstream))
    {
        if (minimum)
        {
            kv.x = (km >= 0.0f) ? std::min(kv.x, km) : std::max(kv.x, km);
        }
        else if (replace)
        {
            kv.x = km;
    ...
    else
    {
        if (minimum)
        {
            kv.y = (km >= 0.0f) ? std::min(kv.y, km) : std::max(kv.y, km);
        }
        else if (replace)
        {
            kv.y = km;
...
Mesh Font::GetGlyphMesh(ushort glyphIndex)
{
    ...
    {
        return Mesh();    // empty / no glyph data
    }
    ...
    {
        return GetComplexMesh(itr);
    }
    else
    {
        throw InvalidFontException("Unknown contourCount in glyph data.");
    }
}
...
Mesh Font::GetSimpleMesh(FItr itr)
{
    ...
    for (ushort i = 0; i < contourCount; ++i)
    {
        ...
        if (endPt >= contours.size())
        {
            contours.resize(endPt + 1);    // assumes zero based index
        }
...
Mesh Font::GetComplexMesh(FItr itr)
{
    ...
    do
    {
        ...
        {
            throw UnsupportedCap("Compound glyph contains point/anchor offsets, which is currently unsupported.");
        }
        ...
        if (abs(abs(a) - abs(c)) <= k)
        {
            m *= 2.0f;
        }

        if (abs(abs(c) - abs(d)) <= k)
        {
            n *= 2.0f;
        }
        ...
        for (auto i = tMesh.begin(); i != tMesh.end(); ++i)
        {
            i->pos = mul(i->pos, mat);    // transform vertices
        }
...
const Mesh& Font::GetTriangulation(CodePoint codePoint)
{
    ...
    // triangulate
    boost::lock_guard<boost::shared_mutex> lock(cacheMutex);
    auto i = meshCache.find(glyphIndex);
...
void Font::PreCache(ulong codeStart, ulong codeEnd, ushort platformId, ushort encodingId, ushort languageId)
{
    ...
    for (ulong i = codeStart; i <= codeEnd; ++i)
    {
        ...
        if (j == meshCache.end())
        {
            meshCache.insert(std::make_pair(glyphIndex, GetGlyphMesh(glyphIndex)));
        }
    }
}
